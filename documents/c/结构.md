...
C语言提供了一个称为 typedef 的功能，它用来建立新的数据类型名。例如，声明
...
typedef 声明并没有创建一个新类型，它只是为某个已经存在的类型添加了一个新的名称而已。

typedef 声明也没有添加任何新的语义，通过这种方式声明的变量与通过声明方式声明的变量具有完全相同的属性。
